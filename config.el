...
;; (defun create-slurm-r-script (&optional time-limit job-name partition modules qos array-range memory cpus-per-task)
...
;; ARRAY-RANGE: array job range, e.g., '1-10' or '1,3,5-8' (optional)
;; MEMORY: memory allocation, e.g., '4G', '8G' (default: prompts user)
;; CPUS-PER-TASK: number of CPUs per task (default: prompts user)"
;;   (interactive)
...
;;          (array-range (or array-range
;;                          (let ((array-input (read-string "Array range (leave empty for no array): " "")))
;;                            (if (string-empty-p array-input) nil array-input))))
;;          (memory (or memory
;;                     (read-string "Memory (e.g., 4G, 8G): " "4G")))
;;          (cpus-per-task (or cpus-per-task
;;                            (read-string "CPUs per task: " "1")))
;;          (output-file (if array-range
;;                          (concat base-name "_%A_%a.out")  ; %A = job ID, %a = array task ID
;;                        (concat base-name "_%j.out")))
;;          (error-file (if array-range
;;                         (concat base-name "_%A_%a.err")
;;                       (concat base-name "_%j.err"))))
...
;; # Run R script with time limit%s as argument%s
;; Rscript %s %s%s%s
...
;;                      cpus-per-task
;;                      memory
;;                      (if array-range " and array task ID" "")
;;                      (if array-range "s" "")
;;                      r-script-name
;;                      time-limit
;;                      (if array-range (format " $SLURM_ARRAY_TASK_ID") "")
;;                      (if (not (string= cpus-per-task "1"))
;;                          (format " $SLURM_CPUS_PER_TASK")
;;                        ""))))
...
;;                (partition (read-string "Partition: " "compute"))
;;                (modules (read-string "Modules (space-separated): " "R"))
